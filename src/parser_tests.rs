...
mod tests {
    ...
    #[test]
    fn test_parse_select_stddev_samp_basic() {
        let result = parse_select("SELECT STDDEV_SAMP(score) FROM users");
        assert!(result.is_ok());
        let (_, cols, _, _, _, _, _, _, _, _) = result.unwrap();
        assert!(cols.is_some());
        let cols = cols.unwrap();
    ...
    #[test]
    fn test_parse_select_var_samp_basic() {
        let result = parse_select("SELECT VAR_SAMP(score) FROM users");
        assert!(result.is_ok());
        let (_, cols, _, _, _, _, _, _, _, _) = result.unwrap();
        assert!(cols.is_some());
        let cols = cols.unwrap();
        assert_eq!(cols[0].to_lowercase(), "var_samp(score)");
    }

    #[test]
    fn test_parse_select_corr_basic() {
        let result = parse_select("SELECT CORR(x,y) FROM users");
        assert!(result.is_ok());
        let (_, cols, _, _, _, _, _, _, _, _) = result.unwrap();
        assert!(cols.is_some());
        let cols = cols.unwrap();
        assert_eq!(cols[0].to_lowercase(), "corr(x,y)");
    }
    ...
    #[test]
    fn test_parse_select_median_basic() {
        // First test tokenization
        let tokens = tokenize("SELECT MEDIAN(salary) FROM employees");
        println!("Tokens: {:?}", tokens);

        let r = parse_select("SELECT MEDIAN(salary) FROM employees");
        if let Err(e) = &r {
    ...
    #[test]
    fn test_parse_select_stddev_pop_basic() {
        let tokens = tokenize("SELECT STDDEV_POP(salary) FROM employees");
        println!("Tokens: {:?}", tokens);

        let r = parse_select("SELECT STDDEV_POP(salary) FROM employees");
        if let Err(e) = &r {
            println!("Parse error: {}", e);
        }
        assert!(r.is_ok());
        ...
        let c = cols.unwrap();
        assert_eq!(c.len(), 1);
        assert_eq!(c[0], "stddev_pop(salary)");
    }
